...
router.get("/wallets", ensureAdmin, async (req, res) => {
    try {
        ...
        return res.render("admin/wallets", { layout: "admin/layout", pageTitle: "Wallets", bitcoinAddress, ethereumAddress, bchAddress, usdtAddress, req });
    } catch (err) {
        return res.redirect("/admin")
    }
});

router.post("/wallets", ensureAdmin, async (req, res) => {
    try {
        const {
            bitcoinAddress,
            ethereumAddress,
        ...
        req.flash("success_msg", "Wallets updated");
